...
import {
  ArrowLeft, Building2, CheckCircle2, Clock, Home,
...
import {
  ...
  getPretermeIrdClientsByMoisKey,
  getAllPretermeIrdImports,
...
function StatBlock({ label, value, sub, accent = false }: {
  ...
  return (
    <div className={cn(
      ...
      <p className={cn(
        "text-2xl font-bold",
        accent ? "text-violet-700 dark:text-violet-300" : "text-slate-900 dark:text-white"
      )}>
        {value}
      </p>
...
export default function HistoriqueIrdDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getPretermeIrdClientsByMoisKey(moisKey),
      getAllPretermeIrdConfigs(),
      getAllPretermeIrdImports(),
    ])
  ...
  return (
    <div className="space-y-6 p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="p-2 bg-violet-950/60 rounded-lg">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Prétermes IARD — fiche synthèse</p>
        </div>
        <Badge className={cn(
          config.valide
      ...
      <Card className="bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-700">
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
            <BarChart3 className="h-4 w-4 text-violet-400" /> Seuils de conservation appliqués
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-4 text-sm text-slate-700 dark:text-slate-300">
            <span className="flex items-center gap-1.5">
              ...
              ETP ≥ <code className="text-violet-600 dark:text-violet-400 font-mono">{config.seuilEtp / 100}</code>
              <span className="text-xs text-slate-500 dark:text-slate-500">(= {config.seuilEtp}%)</span>
            </span>
            <span className="text-slate-400">OU</span>
            <span className="flex items-center gap-1.5">
              <TrendingUp className="h-3.5 w-3.5 text-sky-400" />
              Variation ≥ <code className="text-sky-600 dark:text-sky-400 font-mono">{config.seuilVariation}%</code>
            </span>
          </div>
          {createdAt && (
      ...
      {imports.length > 0 && (
        <Card className="bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-700">
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
              <Building2 className="h-4 w-4 text-violet-400" /> Volumes par agence
            </CardTitle>
          </CardHeader>
      ...
      {parCharge.length > 0 && (
        <Card className="bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-700">
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
              <Users className="h-4 w-4 text-violet-400" /> Répartition par CDC
            </CardTitle>
          </CardHeader>
              ...
              return (
                <div key={prenom} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <Badge className="bg-violet-100 text-violet-700 border-violet-300 dark:bg-violet-900/50 dark:text-violet-300 dark:border-violet-700 text-[10px]">
                      {nb} dossier{nb > 1 ? "s" : ""}
                    </Badge>
                  </div>
      ...
      {historiqueChart.length > 1 && (
        <Card className="bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-700">
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
              <BarChart3 className="h-4 w-4 text-violet-400" /> Historique des cycles IARD (12 derniers mois)
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-48">
                ...
                <BarChart data={historiqueChart} margin={{ top: 4, right: 4, left: -16, bottom: 0 }}>
                  ...
                  <Bar dataKey="globaux" radius={[2, 2, 0, 0]}>
                    {historiqueChart.map((entry, index) => (
                      <Cell
                        key={`cell-g-${index}`}
                        fill={entry.moisKey === moisKey ? "#6d28d9" : "#334155"}
                      />
                    ))}
                  </Bar>
                  <Bar dataKey="conserves" radius={[2, 2, 0, 0]}>
                    {historiqueChart.map((entry, index) => (
                      <Cell
                        key={`cell-c-${index}`}
                        fill={entry.moisKey === moisKey ? "#a78bfa" : "#34d399"}
                      />
                    ))}
                  </Bar>
                </BarChart>
            ...
            <div className="flex items-center gap-4 mt-2 text-[10px] text-slate-500 dark:text-slate-500">
              ...
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-violet-500 inline-block" /> Mois actuel</span>
            </div>
          </CardContent>
        </Card>
      )}

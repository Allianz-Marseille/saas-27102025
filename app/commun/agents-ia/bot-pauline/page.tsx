...
/**
 * Page Pauline — Spécialiste Produits Particuliers (fullscreen).
 * Référence : docs/agents-ia/pauline_retail/
 * Route : /commun/agents-ia/bot-pauline
 */

const SUGGESTIONS_DEMARRAGE = [
  { label: "Règles de souscription (produit)", message: "Quelles sont les règles de souscription pour ce produit particuliers ?" },
  { label: "Documentation produit", message: "Où trouver la documentation produit pour le marché particuliers ?" },
  { label: "Conditions de souscription", message: "Quelles sont les conditions et pièces à fournir pour souscrire ?" },
  { label: "Garanties et exclusions", message: "Quelles garanties et exclusions pour ce contrat particuliers ?" },
  { label: "Infos commerciales", message: "Quels arguments et infos commerciales pour ce produit Allianz particuliers ?" },
] as const;
...
export default function BotPaulinePage() {
  const { user } = useAuth();
  ...
  const handleExportConversationPdf = useCallback(async () => {
    ...
    try {
      ...
      pdf.text("Conversation Pauline — Produits Particuliers", margin, y);
      y += lineHeight + 2;
      pdf.text(
      ...
      for (const msg of messages) {
        ...
        pdf.text(msg.role === "user" ? "Utilisateur :" : "Pauline :", margin, y);
        y += lineHeight;
        pdf.setFont("helvetica", "normal");
      ...
      pdf.text(
        `Généré par Pauline — ${new Date().toLocaleDateString("fr-FR", { dateStyle: "medium" })}`,
        pageWidth / 2,
        pageHeight - 10,
        { align: "center" }
      );
      ...
      if (isMobile) {
        ...
        pdf.save(`pauline-conversation-${new Date().toISOString().slice(0, 10)}.pdf`);
      }
    ...
    const run = async () => {
      ...
      try {
        ...
        if (isMobile) {
          ...
          pdf.save(`pauline-reponse-${pdfExportMessageId.slice(0, 8)}.pdf`);
        }
        ...
        {!hasStarted ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-6 p-8">
            <div className="relative h-32 w-32 overflow-hidden rounded-full border-2 border-emerald-500/30 shadow-md md:h-40 md:w-40">
              <Image
                src="/agents-ia/bot-pauline/pauline.png"
                alt="Pauline, spécialiste produits particuliers"
                fill
            ...
            <p className="max-w-sm text-center text-slate-600 dark:text-slate-400">
              Je suis Pauline, votre spécialiste produits Allianz marché particuliers.
            </p>
            <Button
              size="lg"
          ...
          <div className="flex flex-1 min-h-0">
              ...
              {isLoading && (
                <div className="flex gap-3">
                  ...
                  <div className="flex items-center gap-2 rounded-2xl rounded-tl-sm bg-slate-100 dark:bg-slate-800 px-4 py-3">
                    ...
                    <span className="text-sm text-slate-500">Pauline écrit…</span>
                  </div>
                </div>
              )}
              {error && (
            ...
            <aside className="hidden lg:flex flex-col w-[min(400px,40vw)] shrink-0 border-l border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
              ...
              <div className="flex-1 min-h-0 flex flex-col p-3">
                <Textarea
                  ...
                  placeholder="Le contenu déposé par Pauline apparaît ici…"
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (isMobile) {
                          ...
                          pdf.save("pauline-brouillon.pdf");
                        }
                        toast.success("PDF généré");
                      } catch (e) {
                        console.error(e);
            ...
            {pdfExportMessageId && (() => {
              ...
              return (
                <div
                  ref={pdfExportRef}
                  className="pauline-pdf-export fixed left-[-9999px] top-0 w-[210mm] max-w-[210mm] bg-slate-50 dark:bg-slate-900 p-6 prose prose-sm dark:prose-invert"
                  aria-hidden
                >
                  <MarkdownRenderer content={msg.content} />
                </div>

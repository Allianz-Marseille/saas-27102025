 ...
 * - Format titre : [PRETERME][{AGENCE}][IRD][{MOIS}] {NOM_CLIENT} - {NUM_CONTRAT}
 * - Idempotence : vérifie trelloCardId en base avant création
 * - Rate limiting : délai de 250ms entre chaque carte + retry exponentiel sur 429
 *
 * Différences vs Auto :
 * - Branche = IRD dans titre et description
 * - ETP décimal (1.20 = 20%) — comparaison seuilEtp/100
 * - Pas de Nb sinistres / Bonus-Malus
 * - Champ supplémentaire : Taux d'augmentation indice
 */
...
function formatTitre(
  ...
  return `[PRETERME][${agence}][IRD][${moisKey}] ${nomClient} - ${numeroContrat}`;
}
...
function formatDescription(
  ...
  const seuilEtpDecimal = seuilEtp / 100;
  const anomalies: string[] = [];

  if (client.etp !== null && client.etp !== undefined && client.etp >= seuilEtpDecimal) {
    anomalies.push(`⚠️ **ETP élevé : ${client.etp}** (seuil ≥ ${seuilEtpDecimal.toFixed(2)})`);
  }
  if (
  ...
  const irdClient = client as PretermeClient & { tauxAugmentationIndice?: number | null };

  const lignes = [
    `## Préterme IARD — ${moisKey}`,
    ``,
    ...
    `**ETP :** ${client.etp != null ? client.etp.toFixed(2) : "—"}`,
    `**Taux augmentation indice :** ${irdClient.tauxAugmentationIndice != null ? `${irdClient.tauxAugmentationIndice}%` : "—"}`,
...
export async function dispatcherIrdTrello(

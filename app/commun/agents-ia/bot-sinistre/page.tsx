...
/**
 * Page Sinistro — Assistant agence Sinistres (fullscreen).
 * Référence : docs/agents-ia/sinistro_sinistre/PD_SINISTRO.md, UI-ET-DASHBOARD.md
 * Route : /commun/agents-ia/bot-sinistre
 */

const SUGGESTIONS_DEMARRAGE = [
  { label: "Analyser un constat amiable", message: "Je voudrais analyser un constat amiable (photo ou PDF). Peux-tu extraire les infos et qualifier le sinistre ?" },
  { label: "Qualifier un sinistre (IRSA / IRSI)", message: "Comment qualifier ce sinistre : IRSA, IRSI ou droit commun ? Quels seuils appliquer ?" },
  { label: "Analyse en droit commun", message: "Ce sinistre dépasse les seuils conventionnels. Peux-tu faire une analyse en droit commun (Code des assurances, délais, expertise) ?" },
  { label: "Rédiger un mail de refus de garantie", message: "J'ai besoin d'aide pour rédiger un mail de refus de garantie à un assuré." },
  { label: "Vérifier un recours possible", message: "Vérifier si un recours est possible contre le tiers (auto ou dégâts des eaux)." },
  { label: "Seuils IRSA et IRSI", message: "Quels sont les seuils IRSA (auto) et IRSI (immeuble) à jour ? Quand bascule-t-on en droit commun ?" },
...
export default function BotSinistroPage() {
  const { user } = useAuth();
  ...
  const handleExportConversationPdf = useCallback(async () => {
    ...
    try {
      ...
      pdf.setFont("helvetica", "bold");
                      pdf.text("Conversation Sinistro — Assistant Sinistres", margin, y);
      y += lineHeight + 2;
      pdf.text(
      ...
      for (const msg of messages) {
        ...
        pdf.text(msg.role === "user" ? "Utilisateur :" : "Sinistro :", margin, y);
        y += lineHeight;
        pdf.setFont("helvetica", "normal");
      ...
      pdf.text(
        `Généré par Sinistro — ${new Date().toLocaleDateString("fr-FR", { dateStyle: "medium" })}`,
        pageWidth / 2,
        pageHeight - 10,
        { align: "center" }
      );
      ...
      if (isMobile) {
        ...
        pdf.save(`sinistro-conversation-${new Date().toISOString().slice(0, 10)}.pdf`);
      }
    ...
    const run = async () => {
      ...
      try {
        ...
        if (isMobile) {
          ...
          pdf.save(`sinistro-reponse-${pdfExportMessageId.slice(0, 8)}.pdf`);
        }
  ...
  return (
    <div className="flex h-screen flex-col bg-background">
      ...
      >
        <span>
          Sinistro est un outil d&apos;aide à la décision (conventions IRSA, IRSI, Badinter, droit commun). Les réponses ne constituent pas un conseil juridique personnalisé. Validez la qualification et les montants avec votre process sinistres.
        </span>
        <Link
        ...
        {!hasStarted ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-6 p-8">
            <div className="relative h-32 w-32 overflow-hidden rounded-full border-2 border-teal-500/30 shadow-md md:h-40 md:w-40">
              <Image
                src="/agents-ia/bot-sinistre/sinistro.png"
                alt="Sinistro, assistant sinistres"
                fill
            ...
            <p className="max-w-sm text-center text-slate-600 dark:text-slate-400">
              Je suis Sinistro, votre assistant sinistres. Conventions IRSA, IRSI, Badinter, analyse de constat et droit commun. Réponses sourcées.
            </p>
            <Button
              size="lg"
          ...
          <div className="flex flex-1 min-h-0">
              ...
              {isLoading && (
                <div className="flex gap-3">
                  ...
                  <div className="flex items-center gap-2 rounded-2xl rounded-tl-sm bg-slate-100 dark:bg-slate-800 px-4 py-3">
                    ...
                    <span className="text-sm text-slate-500">Sinistro écrit…</span>
                  </div>
                </div>
              )}
              {error && (
            ...
            <aside className="hidden lg:flex flex-col w-[min(400px,40vw)] shrink-0 border-l border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
              ...
              <div className="flex-1 min-h-0 flex flex-col p-3">
                <Textarea
                  ...
                  placeholder="Le contenu déposé par Sinistro apparaît ici…"
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (isMobile) {
                          ...
                          pdf.save("sinistro-brouillon.pdf");
                        }
                        toast.success("PDF généré");
                      } catch (e) {
                        console.error(e);
            ...
            {pdfExportMessageId && (() => {
              ...
              return (
                <div
                  ref={pdfExportRef}
                  className="sinistro-pdf-export fixed left-[-9999px] top-0 w-[210mm] max-w-[210mm] bg-slate-50 dark:bg-slate-900 p-6 prose prose-sm dark:prose-invert"
                  aria-hidden
                >
                  <MarkdownRenderer content={msg.content} />
                </div>

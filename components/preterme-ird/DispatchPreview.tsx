...
import {
  Send, CheckCircle2, AlertTriangle, Eye, User, Zap
} from "lucide-react";
  ...
  const handleDispatch = async () => {
    ...
    try {
      ...
      if (data.trello.errors === 0) {
        toast.success(`✅ ${data.trello.success} cartes Trello IARD créées avec succès !`);
      } else {
        toast.warning(`${data.trello.success} cartes créées, ${data.trello.errors} erreur(s).`);
      }
    } catch (e) {
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className={cn(
              ...
              <p className={cn(
                "text-xl font-bold",
                statsRoutage.nonRoutes > 0 ? "text-amber-700 dark:text-amber-300" : "text-slate-600 dark:text-slate-400"
              )}>
          ...
          {!trelloMappingOk && (
            <div className="flex items-start gap-2 p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-700/40 rounded-lg text-xs text-red-700 dark:text-red-400">
              ...
              <span>
                ...
                Retournez en Modulation pour renseigner les Board ID et List ID IARD manquants.
              </span>
            </div>
          )}
        </CardContent>
        ...
        <Card className={cn(
          ...
          <CardContent className="pt-4 space-y-3">
            <p className={cn("text-sm font-medium flex items-center gap-2",
              ...
              {result.trello.errors === 0
                ? <><CheckCircle2 className="h-4 w-4" /> Dispatch IARD terminé</>
      ...
      >
        {isDispatching ? (
          <>
            ...
            Création des cartes Trello IARD en cours…
          </>
        ) : (
          <>
            <Send className="h-4 w-4 mr-2" />
            Envoyer {statsRoutage.routes} cartes vers Trello (colonnes IARD)
          </>
        )}
      </Button>
    </div>

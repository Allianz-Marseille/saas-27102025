...
function hasTrelloIncomplete(agences: AgenceConfig[]): boolean {
  return agences.some((a) => a.charges.some((c) => !isTrelloComplete(c.trello)));
}
...
function TrelloForm({ trello, onChange }: { trello: TrelloMapping; onChange: (t: TrelloMapping) => void }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-2 mt-2 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
      ...
      {showPicker && (
        <TrelloListPicker boardId={trello.trelloBoardId} onSelect={handleListSelect} />
      )}
    </div>
  );
}
...
function CdcRow({
  cdc, allCdcs, onUpdate, onDelete, isOnlyOne,
}: {
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-3">
        ...
        <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-600 dark:text-slate-400 hover:text-sky-500"
          onClick={() => setShowTrello((v) => !v)} title="Configurer Trello">
          <Link className="h-3.5 w-3.5" />
        </Button>
        <Button variant="ghost" size="icon"
          className={cn("h-7 w-7", absent ? "text-amber-600 dark:text-amber-400" : "text-slate-600 dark:text-slate-400 hover:text-amber-600")}
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(val) =>
                  onUpdate({ ...cdc, absence: val ? { ...cdc.absence, remplacantId: val } : undefined })
                }
              >
            ...
            <div className="space-y-1">
              ...
              <Input type="date" value={cdc.absence?.dateDebut ?? ""}
                onChange={(e) => onUpdate({ ...cdc, absence: cdc.absence ? { ...cdc.absence, dateDebut: e.target.value } : { remplacantId: "", dateDebut: e.target.value } })}
            ...
            <div className="space-y-1">
              ...
              <Input type="date" value={cdc.absence?.dateFin ?? ""}
                onChange={(e) => onUpdate({ ...cdc, absence: cdc.absence ? { ...cdc.absence, dateFin: e.target.value } : { remplacantId: "", dateFin: e.target.value } })}
                className="h-8 text-xs bg-white border-slate-300 dark:bg-slate-800 dark:border-slate-600" />
            </div>
          </div>
          {cdc.absence && (
...
function AgenceSection({ agence, onChange }: { agence: AgenceConfig; onChange: (updated: AgenceConfig) => void }) {
          ...
          <CardHeader className="cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors rounded-t-lg">
            <CardTitle className="flex items-center justify-between text-base">
              ...
              <div className="flex items-center gap-2">
                ...
                {open ? <ChevronUp className="h-4 w-4 text-slate-600 dark:text-slate-400" /> : <ChevronDown className="h-4 w-4 text-slate-600 dark:text-slate-400" />}
              </div>
            </CardTitle>
          </CardHeader>
        </CollapsibleTrigger>
...
// ─── ConfigurationStep IRD (export principal) ─────────────────────────────────
...
export function ConfigurationStep({
  moisKey, config, onChange, onValidate, onSaveDraft, isLoading, isSavingDraft,
}: ConfigurationStepProps) {
  const trelloBlocked = hasTrelloIncomplete(config.agences);
  const hasEmptyPrenom = config.agences.some((a) => a.charges.some((c) => !c.prenom.trim()));
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-700">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            Seuils de conservation — IARD
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Label className="text-sm text-slate-700 dark:text-slate-300">Seuil ETP (Écart Tarif Portefeuille)</Label>
            ...
            />
            <p className="text-xs text-slate-600 dark:text-slate-500">Valeur par défaut : 120. Conserver si ETP ≥ seuil.</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <Slider
              ...
              min={5} max={50} step={1} className="w-full"
            />
            <p className="text-xs text-slate-600 dark:text-slate-500">Valeur par défaut : 20%. Conserver si Taux de variation ≥ seuil.</p>
          </div>
      ...
      {config.agences.map((agence) => (
        <AgenceSection key={agence.code} agence={agence} onChange={(updated) => updateAgence(agence.code, updated)} />
      ))}
      ...
      {(trelloBlocked || hasEmptyPrenom) && (
        <div className="p-4 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-700/50 rounded-xl space-y-2">
          ...
          {hasEmptyPrenom && (
            <p className="text-xs text-red-700 dark:text-red-300">• Tous les CDC doivent avoir un prénom renseigné.</p>
          )}
          {trelloBlocked && (
            <p className="text-xs text-red-700 dark:text-red-300">
              • Le mapping Trello (Board ID, List ID, URLs) doit être complet pour chaque CDC. Pensez à cibler la colonne &quot;Préterme IARD&quot; de chaque board.
            </p>
          )}
        </div>
      )}
      ...
      <div className="flex flex-col gap-3">
        {onSaveDraft && (
          ...
          >
            {isSavingDraft ? "Sauvegarde..." : (
              <><CheckCircle2 className="h-4 w-4 mr-2 text-amber-400" />Sauvegarder le brouillon</>
            )}
          </Button>
        )}
        <Button
        ...
        >
          {isLoading ? "Validation..." : (
            <><CheckCircle2 className="h-4 w-4 mr-2" />Valider la configuration IARD — {moisKey}</>
          )}
        </Button>
      </div>
    </div>

...
import type { WorkflowIrdState, DispatchStatut } from "@/types/preterme-ird"
...
function buildCdcGroups(workflow: WorkflowIrdState, codeAgence: string, agencies: Agency[]): CdcGroup[] {
  ...
  for (const client of retenus) {
    ...
    if (!cdcMap.has(key)) {
      cdcMap.set(key, {
        ...
        letters: cdc?.letters ?? [],
        // Check pretermeIrd instead of pretermeAuto
        hasConfigError: !cdc || !cdc.boardId || !cdc.lists?.pretermeIrd,
        errorMsg: !cdc
          ? `Lettre "${letter}" non attribuée`
          : !cdc.boardId || !cdc.lists?.pretermeIrd
          ? `Liste pretermeIrd non configurée pour ${cdc.firstName}`
          : null,
        clients: [],
      })
    }
        ...
        <div className="flex items-center gap-2">
          ...
          {group.hasConfigError && (
            <div className="flex items-center gap-1" style={{ fontSize: 10, color: "#f87171" }}>
              ...
              {group.errorMsg ?? "Config Trello manquante"}
            </div>
          )}
        </div>
      </div>
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                <div className="truncate" style={{ fontSize: 11, color: "#f0eeff", lineHeight: 1.3 }}>
                  {client.nomClient}
                </div>
...
export function Step5DispatchIrd({ workflow, onRefresh, onAdvance }: Omit<Props, "onUpdate">) {
  ...
  async function handleDispatch(codeAgence: string, cdcId: string) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/preterme-ird/dispatch", {
        method: "POST",
        headers,
      ...
      if (!res.ok) {
        ...
        toast.error("Erreur dispatch IARD : " + errMsg)
      } else {
        ...
        if (data.erreurs > 0) {
          ...
          toast.success(`✅ ${data.cartesCreees} carte${data.cartesCreees > 1 ? "s" : ""} Trello [IRD] envoyée${data.cartesCreees > 1 ? "s" : ""} avec succès`)
        }
      }
      await onRefresh()
    } catch (e) {
      console.error("Dispatch IARD failed:", e)
      toast.error("Erreur réseau lors du dispatch IARD")
    } finally {
    ...
    <div className="flex flex-col gap-6">
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          >
            <Send style={{ width: 16, height: 16, color: "#ef9f27" }} />
          </div>
          <span style={{
            fontSize: 11, fontWeight: 600, color: "#ef9f27",
        ...
        <p style={{ fontSize: 13, color: "rgba(200,196,230,0.55)", marginTop: 6 }}>
          ...
          Lancez la création des cartes Trello [IRD] CDC par CDC.
        </p>
      </div>

...
import { Upload, Loader2, Lock, Unlock, FileSpreadsheet, CheckCircle2, AlertTriangle, RotateCcw } from "lucide-react"
        ...
        <div className="flex items-center gap-3">
          <div
            ...
            style={{
              width: 32,
              height: 32,
              background: isBlocked ? "rgba(45,197,150,0.1)" : "rgba(239,159,39,0.1)",
              border: `0.5px solid ${isBlocked ? "rgba(45,197,150,0.3)" : "rgba(239,159,39,0.3)"}`,
            }}
          >
            <FileSpreadsheet style={{ width: 14, height: 14, color: isBlocked ? "#2dc596" : "#ef9f27" }} />
          </div>
          <div>
      ...
      <div className="p-5">
        {!isImported && !isBlocked && (
          <div
            ...
            style={{
              height: 100,
              background: dragging ? "rgba(239,159,39,0.08)" : "rgba(255,255,255,0.02)",
              border: `1px dashed ${dragging ? "rgba(239,159,39,0.5)" : "rgba(255,255,255,0.12)"}`,
            }}
          >
            {uploadState.loading ? (
              <Loader2 className="animate-spin" style={{ width: 20, height: 20, color: "#ef9f27" }} />
            ) : (
              <>
                <Upload style={{ width: 20, height: 20, color: "rgba(200,196,230,0.4)" }} />
                <span style={{ fontSize: 12, color: "rgba(200,196,230,0.4)" }}>
                  Déposez le fichier IARD {codeAgence}...xlsx
                </span>
              </>
            )}
            <input
          ...
          <>
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                ...
                { label: "Retenus", value: clientsRetenus, color: "#ef9f27" },
            ...
            >
              ...
              <div className="flex flex-col gap-4">
                <div>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: "#ef9f27", fontFamily: "DM Mono, monospace" }}>
                      {localMajo}%
                    </span>
                  </div>
                  <input
              ...
              {recalcing && (
                <div className="flex items-center gap-2 mt-3">
                  <Loader2 className="animate-spin" style={{ width: 10, height: 10, color: "#ef9f27" }} />
            ...
            >
              {uploadState.loading ? (
                <Loader2 className="animate-spin" style={{ width: 12, height: 12, color: "#ef9f27" }} />
              ) : (
                <>
...
export function Step2ImportIrd({ workflow, onUpdate, onRefresh, onAdvance }: Props) {
  ...
  async function handleRecalc(codeAgence: string, seuilMajo: number, seuilEtp: number) {
    try {
      const token = (await buildAuthenticatedJsonHeaders()).Authorization
      await fetch("/api/admin/preterme-ird/recalc", {
        method: "POST",
    ...
    <div className="flex flex-col gap-6">
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          >
            <Upload style={{ width: 16, height: 16, color: "#ef9f27" }} />
          </div>
          <span style={{ fontSize: 11, fontWeight: 600, color: "#ef9f27", letterSpacing: "0.08em", fontFamily: "DM Mono, monospace" }}>
            ÉTAPE 2
          </span>
        </div>

        <h3 className="font-bold" style={{ fontSize: 20, color: "#f0eeff", fontFamily: "Syne, sans-serif" }}>
          Import &amp; Filtrage — IARD
        </h3>
        <p style={{ fontSize: 13, color: "rgba(200,196,230,0.55)", marginTop: 6 }}>
          Importez le fichier Excel IARD de chaque agence, ajustez les seuils de rétention (règle OU), puis bloquez chaque agence pour valider.
        </p>
      </div>

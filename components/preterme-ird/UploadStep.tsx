...
import {
  ...
  RefreshCw, X, Info, Building2, Loader2,
...
function DropZone({
  onFiles, disabled, compact = false,
}: {
  ...
  return (
    ...
    >
      <input
        ...
        onChange={(e) => { const sel = Array.from(e.target.files ?? []); if (sel.length) onFiles(sel); e.target.value = ""; }}
        disabled={disabled}
      />
      <div className={cn("flex flex-col items-center gap-2 transition-transform duration-200", isDragging && "scale-105")}>
        <div className={cn("rounded-full p-3 transition-colors duration-200",
          ...
          <Upload className={cn("transition-colors duration-200", compact ? "h-5 w-5" : "h-7 w-7",
        ...
        {!compact ? (
          <>
            <p className="text-sm font-medium text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-slate-200 transition-colors">
              Glissez un ou plusieurs fichiers Excel IARD, ou cliquez pour sélectionner
            </p>
...
function FileCard({
  state, onAgenceChange, onUpload, onRemove,
}: {
    ...
    <div className={cn(
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-500 mt-0.5">{(state.file.size / 1024).toFixed(0)} Ko</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {isUploading && <Loader2 className="h-4 w-4 text-sky-600 dark:text-sky-400 animate-spin" />}
          {state.status === "success" && <CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />}
          {!isUploading && (
            <Button variant="ghost" size="icon" className="h-6 w-6 text-slate-500 hover:text-red-600 dark:hover:text-red-400" onClick={onRemove}>
      ...
      {state.agenceDetectee ? (
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            ...
            <Select value={state.agenceSelectionnee ?? ""} onValueChange={(v) => onAgenceChange(v as AgenceCode)} disabled={isUploading}>
      ...
      {state.status === "success" && state.result && (
        <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 rounded-lg border border-emerald-200 dark:border-emerald-800/40 space-y-1.5">
          <p className="text-xs font-medium text-emerald-700 dark:text-emerald-300 flex items-center gap-1.5">
            <CheckCircle2 className="h-3.5 w-3.5" /> Import IARD réussi
          </p>
      ...
      {!isUploading && state.status !== "success" && (
        <Button size="sm" onClick={onUpload} disabled={!agenceFinal}
      ...
      {state.status === "success" && (
        <Button variant="outline" size="sm" onClick={onUpload}
          ...
          <RefreshCw className="h-3.5 w-3.5 mr-1.5" /> Réimporter (remplacera les données existantes)
        </Button>
      )}
    </div>
  );
...
function GlobalStatusBanner({
  agencesImportees, agencesManquantes, total,
}: {
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-1">
        {allDone ? (
          <span className="font-medium">Les 2 agences IARD sont importées — vous pouvez continuer</span>
        ) : (
          <span>
...
// ─── UploadStep IRD (export principal) ────────────────────────────────────
...
export function UploadStep({ moisKey, configValide, idToken, onImportSuccess }: UploadStepProps) {
  ...
  const [replaceConfirm, setReplaceConfirm] = useState<{ idx: number; agence: AgenceCode } | null>(null);
  ...
  const addFiles = (incoming: File[]) => {
    setFiles((prev) => {
      ...
      if (duplicates.length) toast.warning(`Fichier(s) déjà présent(s) : ${duplicates.join(", ")}`);
      return [...prev, ...toAdd];
    });
  };
  ...
  const doUpload = async (idx: number) => {
    ...
    try {
      const res = await fetch("/api/admin/preterme-ird/upload", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        toast.error(data.error ?? "Erreur lors de l'import IARD");
        return;
      }

      updateFile(idx, { status: "success", result: data });
      toast.success(`Import IARD réussi — ${data.nbLignesValides} clients chargés`);
  ...
  const uploadFile = (idx: number) => {
    ...
    if (alreadyImported) { setReplaceConfirm({ idx, agence }); return; }
    void doUpload(idx);
  };
  ...
  const importAllReadyFiles = () => {
    ...
    if (readyIndexes.length === 0) { toast.warning("Aucun fichier prêt à importer."); return; }
    void Promise.all(readyIndexes.map((idx) => doUpload(idx)));
  };

  const successFiles    = files.filter((f) => f.status === "success");
  const agencesImportees = new Set(successFiles.map((f) => f.agenceDetectee ?? f.agenceSelectionnee));
  const agencesManquantes = (["H91358", "H92083"] as AgenceCode[]).filter((a) => !agencesImportees.has(a));
    ...
    <div className="space-y-4">
      {!configValide && (
        <div className="p-4 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-700/50 rounded-xl flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-amber-700 dark:text-amber-300">Période non confirmée</p>
            <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
              Confirmez d&apos;abord la période (étape 1) avant d&apos;importer les fichiers IARD.
            </p>
          </div>
        </div>
      )}

      <GlobalStatusBanner agencesImportees={agencesImportees} agencesManquantes={agencesManquantes} total={files.length} />
      ...
      {files.length === 0 && configValide && (
        <div className="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-600 dark:text-slate-400 space-y-1">
          <p className="flex items-center gap-1.5 font-medium text-slate-700 dark:text-slate-300">
            <Info className="h-3.5 w-3.5" /> Deux fichiers IARD attendus
          </p>
          {(Object.entries(AGENCES) as [AgenceCode, { nom: string; label: string }][]).map(([code, info]) => (
      ...
      {showDropzone && (
        <DropZone onFiles={addFiles} disabled={!configValide} compact={files.length > 0} />
      )}
      ...
      {files.length > 1 && hasReadyToUpload && (
        <Button size="sm" onClick={importAllReadyFiles} className="w-full bg-sky-700 hover:bg-sky-600" disabled={isAnyUploading}>
          {isAnyUploading ? (
            <><Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" /> Import en cours...</>
          ) : (
            <><Upload className="h-3.5 w-3.5 mr-1.5" /> Importer tous les fichiers IARD prêts</>
          )}
        </Button>
      )}
        ...
        <Card className="bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-700">
          <CardHeader className="pb-2 pt-4 px-4">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              Récapitulatif des imports IARD
            </CardTitle>
          </CardHeader>
      ...
      <AlertDialog open={!!replaceConfirm} onOpenChange={(open) => { if (!open) setReplaceConfirm(null); }}>
        <AlertDialogContent className="bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-700">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-slate-900 dark:text-slate-100">Agence déjà importée</AlertDialogTitle>
            <AlertDialogDescription className="text-slate-600 dark:text-slate-400">
              Un import IARD existe déjà pour{" "}

...
function buildCdcRows(workflow: WorkflowIrdState, code: string, agencies: Agency[]): CdcRow[] {
...
export function Step6SlackIrd({ workflow, onUpdate }: Props) {
    ...
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <div className="flex items-center justify-center rounded-lg" style={{ width: 36, height: 36, background: "rgba(239,159,39,0.1)", border: "0.5px solid rgba(239,159,39,0.3)" }}>
            <MessageSquare style={{ width: 16, height: 16, color: "#ef9f27" }} />
          </div>
          <span style={{ fontSize: 11, fontWeight: 600, color: "#ef9f27", letterSpacing: "0.08em", fontFamily: "DM Mono, monospace" }}>ÉTAPE 6</span>
        </div>
        <h3 className="font-bold" style={{ fontSize: 20, color: "#f0eeff", fontFamily: "Syne, sans-serif" }}>Rapport Slack IARD</h3>
        <p style={{ fontSize: 13, color: "rgba(200,196,230,0.55)", marginTop: 6 }}>
          Envoyez la synthèse du traitement IARD sur Slack. Sélectionnez la chaîne cible avant d&apos;envoyer.
        </p>
      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-1 gap-y-0.5" style={{ paddingLeft: 4 }}>
                      {a.cdcRows.map((row, i) => (
                        <span key={i} style={{ fontSize: 11, color: "rgba(200,196,230,0.7)" }}>
                          <span style={{ color: "#ef9f27" }}>{row.prenom}</span>
                          {": "}
        ...
        <div className="px-4 py-3" style={{ borderBottom: "0.5px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              style={{
                fontSize: 11,
                background: showPicker ? "rgba(239,159,39,0.1)" : "rgba(255,255,255,0.04)",
                border: `0.5px solid ${showPicker ? "rgba(239,159,39,0.3)" : "rgba(255,255,255,0.1)"}`,
                color: showPicker ? "#ef9f27" : "rgba(200,196,230,0.5)",
          ...
          <div className="flex items-center gap-2">
            <Hash style={{ width: 13, height: 13, color: "#ef9f27" }} />
            ...
            <div style={{ maxHeight: 200, overflowY: "auto" }}>
              {loadingChannels && channels.length === 0 ? (
                <div className="flex items-center justify-center py-5 gap-2">
                  <Loader2 style={{ width: 13, height: 13, color: "#ef9f27" }} className="animate-spin" />
                ...
                filteredChannels.map(ch => {
                  ...
                  return (
                    <button
                      ...
                      style={{ background: isSelected ? "rgba(239,159,39,0.08)" : "transparent", borderBottom: "0.5px solid rgba(255,255,255,0.03)" }}
                      onMouseEnter={e => { if (!isSelected) (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.03)" }}
                      onMouseLeave={e => { if (!isSelected) (e.currentTarget as HTMLElement).style.background = isSelected ? "rgba(239,159,39,0.08)" : "transparent" }}
                    >
                      <div className="flex items-center gap-2">
                        <Hash style={{ width: 11, height: 11, color: isSelected ? "#ef9f27" : "rgba(200,196,230,0.3)" }} />
        ...
        <div className="px-4 py-3">
          <button
            ...
            style={{
              ...
              color: sending ? "rgba(200,196,230,0.3)" : alreadySent ? "#c4b5fd" : "#ef9f27",
              fontSize: 14,
              fontFamily: "Syne, sans-serif",
              boxShadow: (!sending && !alreadySent) ? "0 0 16px rgba(239,159,39,0.12)" : "none",

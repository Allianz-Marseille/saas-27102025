...
export function HistoPanelIrd({ allWorkflows, onClose }: Props) {
      ...
      <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "0.5px solid rgba(255,255,255,0.07)" }}>
        <div className="flex items-center gap-2">
          <div
            ...
            style={{ width: 28, height: 28, background: "rgba(239,159,39,0.1)", border: "0.5px solid rgba(239,159,39,0.25)" }}
          >
            <TrendingUp style={{ width: 13, height: 13, color: "#ef9f27" }} />
          </div>
        ...
        {(["importes", "retenus"] as Tab[]).map(t => (
          <button
            ...
            style={{
              fontSize: 11,
              fontFamily: "DM Mono, monospace",
              background: tab === t ? "rgba(239,159,39,0.12)" : "rgba(255,255,255,0.03)",
              border: `0.5px solid ${tab === t ? "rgba(239,159,39,0.4)" : "rgba(255,255,255,0.07)"}`,
              color: tab === t ? "#ef9f27" : "rgba(200,196,230,0.45)",
            }}
          >
        ...
        {allWorkflows.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <span style={{ fontSize: 12, color: "rgba(200,196,230,0.3)", fontFamily: "DM Mono, monospace" }}>
              Aucun workflow IARD disponible
            </span>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              />
              {AGENCES.map(code => (
                <Bar key={code} dataKey={code} fill={COLORS[code]} radius={[3, 3, 0, 0]} opacity={0.85} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        )}

...
import { calculerIrdStatsConservation } from "@/lib/services/preterme-ird-anomaly";
...
function AgencePanel({
  ...
  const stats = useMemo(
    () => calculerIrdStatsConservation(imp.clients, state.seuilEtp, state.seuilVariation),
    [imp.clients, state.seuilEtp, state.seuilVariation]
  );

  // Affichage ETP IARD en décimal : 120 → 1.20
  const seuilEtpDecimal = (state.seuilEtp / 100).toFixed(2);
      ...
      <Card className="bg-slate-900 border-slate-700">
        ...
        <CardContent className="space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-0.5">ETP IARD en décimal — 1.20 = +20%</p>
              </div>
          ...
          <div className="p-3 bg-slate-800/60 rounded-lg border border-slate-700 text-xs text-slate-400">
            <span className="font-medium text-slate-300">Règle IARD :</span>{" "}
            conserver si{" "}
      ...
      {state.locked ? (
        <div className="flex items-center gap-3">
          <div className="flex-1 flex items-center gap-2 p-3 bg-emerald-950/30 border border-emerald-800/50 rounded-lg text-sm text-emerald-300">
            ...
            <span>Validé — ETP ≥ {seuilEtpDecimal} | Majo ≥ {state.seuilVariation}%</span>
          </div>
          <Button

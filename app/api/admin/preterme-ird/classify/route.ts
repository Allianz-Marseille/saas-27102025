 ...
 *
 * 1. Lit les clients d'un import IRD (importId).
 * 2. Applique le filtrage métier (ETP >= seuilEtp/100 OR TauxVariation >= seuilVariation).
 ...
 *   ou
 *   { importIds: string[], seuilEtp: number, seuilVariation: number }
 *
 * Note ETP : l'ETP IARD est stocké en décimal (1.20 = 20%). La comparaison utilise seuilEtp/100.
 */

import { NextRequest, NextResponse } from "next/server";
import { verifyAdmin } from "@/lib/utils/auth-utils";
import { adminDb, Timestamp } from "@/lib/firebase/admin-config";
import { doitEtreIrdConserve } from "@/lib/services/preterme-ird-anomaly";
...
async function getQualityOverrideMap(
...
export async function POST(request: NextRequest) {
  ...
  const targetImportIds = Array.from(
    new Set(
      (importIds && importIds.length > 0 ? importIds : importId ? [importId] : []).filter(Boolean)
    )
  ) as string[];
  ...
  for (const currentImportId of targetImportIds) {
    const importRef = adminDb.collection("preterme_iard_imports").doc(currentImportId);
    ...
    // Filtrage métier IRD : ETP décimal => seuilEtp/100
    ...
    // Quality overrides (mémoire partagée Auto + IRD)
    ...
    // Clients à envoyer à Gemini (pas d'override)
    ...
    // Résultats combinés (override + Gemini)
    ...
    for (let i = 0; i < clients.length; i += BATCH_SIZE) {
      ...
      for (const c of slice) {
        ...
        const ref = adminDb.collection("preterme_iard_clients").doc(c.id);
    ...
    results.push({
      ...
      ratioConservation:
        clients.length > 0 ? Math.round((conserves.length / clients.length) * 100) : 0,
    });
  }
